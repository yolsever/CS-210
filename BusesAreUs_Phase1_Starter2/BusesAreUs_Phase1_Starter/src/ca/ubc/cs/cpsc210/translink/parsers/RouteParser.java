package ca.ubc.cs.cpsc210.translink.parsers;

import ca.ubc.cs.cpsc210.translink.model.Route;
import ca.ubc.cs.cpsc210.translink.model.RouteManager;
import ca.ubc.cs.cpsc210.translink.model.RoutePattern;
import ca.ubc.cs.cpsc210.translink.parsers.exception.RouteDataMissingException;
import ca.ubc.cs.cpsc210.translink.providers.DataProvider;
import ca.ubc.cs.cpsc210.translink.providers.FileDataProvider;
import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

import java.io.IOException;

/**
 * Parse route information in JSON format.
 */
public class RouteParser {
    private String filename;

    public RouteParser(String filename) {
        this.filename = filename;
    }
    /**
     * Parse route data from the file and add all route to the route manager.
     *
     */
    public void parse() throws IOException, RouteDataMissingException, JSONException{
        DataProvider dataProvider = new FileDataProvider(filename);

        parseRoutes(dataProvider.dataSourceToString());
    }
    /**
     * Parse route information from JSON response produced by Translink.
     * Stores all routes and route patterns found in the RouteManager.
     *
     * @param  jsonResponse    string encoding JSON data to be parsed
     * @throws JSONException   when:
     * <ul>
     *     <li>JSON data does not have expected format (JSON syntax problem)
     *     <li>JSON data is not an array
     * </ul>
     * If a JSONException is thrown, no stops should be added to the stop manager
     *
     * @throws RouteDataMissingException when
     * <ul>
     *  <li>JSON data is missing RouteNo, Name, or Patterns element for any route</li>
     *  <li>The value of the Patterns element is not an array for any route</li>
     *  <li>JSON data is missing PatternNo, Destination, or Direction element for any route pattern</li>
     * </ul>
     * If a RouteDataMissingException is thrown, all correct routes are first added to the route manager.
     */

    public void parseRoutes(String jsonResponse)
            throws JSONException, RouteDataMissingException {
        // TODO: Task 4: Implement this method
        //completed all

        JSONArray busRoutes = new JSONArray(jsonResponse);
        boolean didItThrow = false;

        for (int i = 0; i < busRoutes.length(); i++)
        {
            try{
            JSONObject ithroute = busRoutes.getJSONObject(i);
            readRoutes(ithroute);
        }catch (RouteDataMissingException e){
               didItThrow = true;
            }
        }

        if(didItThrow)
            throw new RouteDataMissingException();
    }


    public void readRoutes(JSONObject ithroute) throws JSONException, RouteDataMissingException {
            try {
                  String name = ithroute.getString("Name");
                  //patterns have an array in it about more info(dest,direc,etc)
                  JSONArray patterns = ithroute.getJSONArray("Patterns");
                  String routeNo = ithroute.getString("RouteNo");
                  Route newRoute = RouteManager.getInstance().getRouteWithNumber(routeNo, name);
                      patternParser(patterns, newRoute);
              }catch (JSONException e){
                  throw new RouteDataMissingException();
              }

    }

    public void patternParser(JSONArray patterns, Route newRoute) throws JSONException, RouteDataMissingException {
        boolean diditthrow = false;

        for (int i = 0; i < patterns.length(); i++) {
            try {
                JSONObject ithSubPattern = patterns.getJSONObject(i);
                readSubs(ithSubPattern, newRoute);
            } catch (RouteDataMissingException e) {
                diditthrow = true;
               //throw new RouteDataMissingException();
            }
        }

        if(diditthrow)
            throw new RouteDataMissingException();
    }


    public void readSubs(JSONObject ithSubPattern, Route newRoute) throws JSONException, RouteDataMissingException{
        //boolean didItThrow3 = false;
        try{
            String destination = ithSubPattern.getString("Destination");
        String direction = ithSubPattern.getString("Direction");
        String patternNo = ithSubPattern.getString("PatternNo");
        //routeMap has a Href stuff in it, but for some reason we dont care
        RoutePattern pattern = new RoutePattern(patternNo, destination, direction, newRoute);
        newRoute.addPattern(pattern);
        }catch (JSONException e){
            throw new RouteDataMissingException();
            //didItThrow3 = true;
        }


    }

//      apparently unnecesarry
//    public void routeMapParser(JSONObject routeMap) throws JSONException{
//        JSONObject routeMapSub  = new JSONObject(routeMap);
//
//        for (int i = 0; i < routeMapSub.length(); i++)
//        {
//            JSONObject ithSubMap = routeMapSub.getJSONObject(i);
//        }
//    }



}
